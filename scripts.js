...
function generateCardsArray(numCards) {
    ...
    for (let i = 1; i <= numPairs; i++) {
        ...
        switch (theme) {
            case 'amorcito':
                symbol = `amorcito/i${i}.jpeg`; // Cambia esto a tu ruta de imagen
                break;
            case 'emojis':
...
function revealCard() {
    ...
    // Verifica si el tema es amorcito y carga la imagen
    if (document.querySelector('.theme.selected').getAttribute('data-theme') === 'amorcito') {
        const img = document.createElement('img');
        img.src = symbol; // Asigna la ruta de la imagen
        img.style.width = '100%'; // Ajusta la imagen al tamaño del contenedor
        img.style.height = '100%';
        this.appendChild(img); // Agrega la imagen al div
    } else {
        this.textContent = symbol; // Para otros temas, solo muestra el símbolo
    }
